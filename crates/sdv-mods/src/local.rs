use crate::model::{Mod, ModConfigField};
...
/// 解析并读取宽松 JSON 文件。
pub fn read_loose_json(path: &Path) -> Option<Value> {
    let content = fs::read_to_string(path).ok()?;
    serde_json::from_str(&clean_json_content(&content)).ok()
}

/// 读取宽松 JSON 文件，解析失败或文件不存在时回退到空对象。
pub fn read_loose_json_object(path: &Path) -> Value {
...
pub fn parse_i18n_key(value: &str) -> Option<String> {
...
pub fn first_clean_text(candidates: &[&str]) -> Option<String> {
...
pub fn parse_mod_folder(mod_dir: &Path, mods_root: &Path) -> Result<Mod, String> {
...
pub fn validate_mod_folder_name(folder_name: &str) -> Result<(), String> {
...
pub fn resolve_mod_dir(game_dir: &str, folder_name: &str) -> Result<PathBuf, String> {
...
/// 扫描 `<game_dir>/Mods` 下的全部模组；目录不存在时返回空列表。
pub fn list_installed_mods(game_dir: &str) -> Result<Vec<Mod>, String> {
...
/// 通过给文件夹名加/去前缀点号来禁用/启用模组，返回改名后的相对路径。
pub fn toggle_mod(game_dir: &str, folder_name: &str, enable: bool) -> Result<String, String> {
    let mods_dir = Path::new(game_dir).join("Mods");
    if !mods_dir.exists() {
...
pub fn delete_mod(game_dir: &str, folder_name: &str) -> Result<(), String> {
...
pub fn save_mod_config(game_dir: &str, folder_name: &str, config: &Value) -> Result<(), String> {
...
fn read_manifest_object(mod_dir: &Path) -> Result<(PathBuf, Value), String> {
    let manifest_path = mod_dir.join("manifest.json");
...
/// 把译文以字面量写进 manifest，并把原文备份到 i18n/default.json。
...
pub fn rename_local_mod(game_dir: &str, folder_name: &str, new_name: String) -> Result<(), String> {
    let mod_dir = resolve_mod_dir(game_dir, folder_name)?;
    let (manifest_path, mut manifest_val) = read_manifest_object(&mod_dir)?;
