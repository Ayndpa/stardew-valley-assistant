...
/// 落盘缓存的目录布局：`<cache_dir>/cache/fishing_maps.json` 与 `<cache_dir>/cache/previews/*.png`。
/// 保持这层 `cache/` 子目录是为了兼容桌面端已有的磁盘缓存，升级后老缓存仍然命中。
fn get_fishing_cache_path(cache_dir: &Path) -> Result<PathBuf, String> {
...
fn get_preview_cache_path(
    ...
    let previews_dir = cache_dir.join("cache").join("previews");
...
fn load_maps_cache_from_disk(cache_dir: &Path) -> Option<CachedFishingMaps> {
    let path = get_fishing_cache_path(cache_dir).ok()?;
    if !path.exists() {
        return None;
    }
...
fn save_maps_cache_to_disk(cache_dir: &Path, cache: &CachedFishingMaps) {
    if let Ok(path) = get_fishing_cache_path(cache_dir) {
...
fn load_preview_cache_from_disk(
    ...
    let path = get_preview_cache_path(cache_dir, cache_key, map_id, fingerprint).ok()?;
...
fn save_preview_png_to_disk(
    ...
    let path = get_preview_cache_path(cache_dir, cache_key, map_id, fingerprint)?;
    fs::write(&path, png).map_err(|e| format!("写入地图底图缓存失败: {}", e))?;
    Ok(path.to_string_lossy().to_string())
}

/// 钓鱼地图总览：只含摘要，不渲染底图。
pub fn build_fishing_map_data(
    cache_dir: &Path,
    content_dir: &Path,
    force_refresh: bool,
) -> Result<FishingMapData, String> {
    let (cache, cached) = get_or_build_fishing_map_cache(cache_dir, content_dir, force_refresh)?;
    let maps = cache
        .maps
        .iter()
...
/// 单张钓鱼地图详情：含可钓格子、鱼群区域与底图预览路径。
///
/// `mod_prices` 由调用方从运行时导出文件里读出；为 None 时一律回退 XNB 价格。
pub fn build_fishing_map_detail(
    cache_dir: &Path,
    content_dir: &Path,
    map_id: &str,
    force_refresh: bool,
    ...
    detail.fishing_areas = load_fishing_areas_for_map(content_dir, &detail.id, lang, mod_prices)?;
...
fn fingerprint_maps_dir(
    ...
    let paths = crate::collect_xnb_files(&content_dir.join("Maps"))?;
...
pub fn get_or_build_fishing_map_cache(
    ...
    if force_refresh {
        ...
        // Try disk cache next
        if let Some(disk_cache) = load_maps_cache_from_disk(cache_dir) {
    ...
    // Save to disk cache
    save_maps_cache_to_disk(cache_dir, &built);
...
pub fn get_or_render_fishing_map_preview(
    ...
    if let Some(disk_preview) =
        load_preview_cache_from_disk(cache_dir, cache_key, map_id, fingerprint)
    {
    ...
    let preview = match render_tbin_map_preview_png(content_dir, &map_path) {
        Ok(png) => match save_preview_png_to_disk(cache_dir, cache_key, map_id, fingerprint, &png) {
...
pub fn load_fishing_areas_for_map(
    content_dir: &Path,
    map_id: &str,
    lang: Option<&str>,
    mod_prices: Option<&HashMap<String, i32>>,
