//! 星露谷游戏数据解析。
//!
//! 这里只放「给定 Content 目录就能算出结果」的纯逻辑：XNB / tBIN 解包、贴图裁切、
//! 各类百科数据构建。所有入口都显式接收 `content_dir`，不做任何游戏安装目录探测，
//! 也不读取 %APPDATA% 下的运行时导出文件——那些依赖平台的部分留在各自的宿主应用里，
//! 桌面端与手机端因此可以共用同一份解析实现。
...
/// 递归收集目录下的全部 .xnb 文件，结果按路径排序。
///
/// 排序是必需的：钓鱼地图缓存的指纹与条目顺序都依赖它，`read_dir` 的返回顺序
/// 在不同平台/文件系统上并不稳定。
...
mod tests {
    ...
    fn debug_white_cow() {
        ...
        for _ in 0..count {
            ...
            if key == "White Cow" {
                eprintln!("=== White Cow starts at {} (value reader {}) ===", start, vr);
                ...
                if egg_r != 0 {
                    ...
                    for _ in 0..ec { let _ = reader.read_object_string_any().unwrap(); }
