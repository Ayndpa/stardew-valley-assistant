...
use super::image_utils::{resolve_object_texture_path, object_texture_key, Rect, Texture};
...
pub fn harvest_type_name(harvest_type: i32, is_zh: bool) -> String {
    match harvest_type {
        0 => {
            if is_zh {
                "夜间掉落".to_string()
...
pub fn house_display_name(house: &str, is_zh: bool) -> String {
    match house {
        "Coop" => {
            if is_zh {
                "鸡舍".to_string()
...
pub fn resolve_localized_name(raw: &str, strings: &HashMap<String, String>) -> String {
...
/// 从 XNB 文件解析动物数据（含图标）
pub fn build_animal_data_from_xnb(
    content_dir: &Path,
    lang_str: &str,
    is_zh: bool,
