...
/// 从 Content 目录解析社区中心收集包（含配料与奖励图标）。
pub fn build_bundle_game_data_from_xnb(
    content_dir: &Path,
    lang: Option<&str>,
) -> Result<BundleGameData, String> {
    let lang_suffix = get_lang_suffix(lang);
    let is_zh = lang.is_some_and(|l| l.to_lowercase().starts_with("zh"));
    ...
    let bundle_names_tables =
        load_localized_string_tables_with_lang(content_dir, &["BundleNames"], lang);
...
mod tests {
    ...
    fn bundles_xnb_is_string_dictionary() {
        ...
        let raw = crate::xnb::load_string_dictionary_xnb(&path)
    ...
    fn reward_is_parsed() {
        ...
        let data = build_bundle_game_data_from_xnb(&content, Some("zh")).unwrap();
    ...
    fn loads_bundle_game_data() {
        ...
        let result = build_bundle_game_data_from_xnb(&content, Some("zh"));
        match result {
            Ok(data) => {
