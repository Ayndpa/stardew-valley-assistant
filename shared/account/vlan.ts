/**
 * 虚拟局域网（REALTIME.md §6）：封装 Tauri 命令 vlan_start / vlan_update_members / vlan_signal_in / vlan_stop /
 * vlan_status / vlan_helper_status 与事件 vlan-event / vlan-signal-out。
 * 只在主视图的 SocialProvider 里使用（需要房间上下文与 room.signal 通道）。
 *
 * 进入房间（room.ready）后自动 vlan_start（localStorage `vlanAutoJoin === "false"` 时除外）；
 * 启动失败不会自动重试，由面板上的「重试」触发。
 * Rust 侧命令可能尚未就绪或在浏览器预览中不存在——所有 invoke 错误都被捕获并以 error 呈现。
 *
 * 两端命令名与事件名完全一致，唯一的平台差异是"授权失败"的错误文本前缀
 * （桌面端是 UAC 提权，手机端是 Android VpnService 授权），以及手机端多一步 vlan_prepare。
 * 这些差异通过 VlanConfig 注入，本文件本身不区分平台。
 */
...
export interface VlanConfig {
  /** 用户拒绝授权时 vlan_start 错误文本的前缀 */
  deniedPrefix: string
  /** 缺少前置权限时错误文本的前缀 */
  needsPermissionPrefix: string
  /** vlan_start 之前需要先调用的准备命令；不需要时省略 */
  prepareCommand?: string
}

/** 桌面端：提权辅助进程 + UAC（REALTIME.md §6.4） */
export const DESKTOP_VLAN_CONFIG: VlanConfig = {
  deniedPrefix: "未获得管理员权限",
  needsPermissionPrefix: "需要以管理员身份运行",
}

/** 手机端：Android VpnService 授权，vlan_start 前先 vlan_prepare 申请授权 */
export const MOBILE_VLAN_CONFIG: VlanConfig = {
  deniedPrefix: "未获得 VPN 授权",
  needsPermissionPrefix: "需要 VPN 授权",
  prepareCommand: "vlan_prepare",
}
...
interface UseVlanParams {
  /** 只有主视图（有房间能力）才激活桥接；平台未接入虚拟局域网时传 false */
  active: boolean
  /** 平台差异注入，见 VlanConfig */
  config: VlanConfig
  ...
  /** 错误提示（主视图 toast）；kind=denied 表示用户拒绝授权，调用方可换成 i18n 文案 */
...
export function classifyVlanError(error: string | null, config: VlanConfig): VlanErrorKind | null {
  if (!error) return null
  if (error.startsWith(config.deniedPrefix)) return "denied"
  if (error.startsWith(config.needsPermissionPrefix)) return "needsAdmin"
  return "other"
}
...
export function useVlan({ active, config, roomCode, you, members, sendSignal, onError }: UseVlanParams): VlanController {
  ...
  const refreshHelperStatus = useCallback(async () => {
    ...
    try {
      const next = await tauriInvoke<Record<string, unknown>>("vlan_helper_status")
      if (next && typeof next === "object") {
        setHelper({
          elevated: !!next.elevated,
          // 桌面端 Rust 侧序列化为 helper_running，手机端为 helperRunning，两种都认
          helper_running: !!(next.helper_running ?? next.helperRunning),
          // 桌面端不返回该字段：缺省即视为支持
          available: next.available === undefined ? true : !!next.available,
        })
      }
    } catch {
      // 命令不可用时保持未知
  ...
  const enable = useCallback(async () => {
    ...
    try {
      // 手机端需要先申请 VpnService 授权（vlan_prepare 返回 true 表示已授权）；桌面端不配置该命令
      const prepare = configRef.current.prepareCommand
      if (prepare) await tauriInvoke<boolean>(prepare)
      ...
      onErrorRef.current(text, classifyVlanError(text, configRef.current) ?? "other")
    } finally {
      startingRef.current = false
      setStarting(false)
      setStatusText(null)
  ...
  const errorKind = useMemo(() => classifyVlanError(error, config), [error, config])
