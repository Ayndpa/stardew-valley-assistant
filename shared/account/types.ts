...
/**
 * vlan_helper_status。
 * 桌面端：elevated = 应用已提权，helper_running = 提权辅助进程在跑（REALTIME.md §6.4）。
 * 手机端：elevated = 已拿到 VPN 授权，helper_running = 引擎在跑，
 * available = 当前平台支持虚拟局域网（桌面预览窗口里为 false；桌面端不返回该字段，按 true 处理）。
 */
